...
patch(PosStore.prototype, {
    ...
    async addLineToOrder(vals, order, opts = {}, configure = true) {
        ...
        // START NEW
        if (values.product_id.isConfigurable() && configure || values.product_id.raw.has_design) {
            const payload = await this.openConfigurator(values.product_id,values.product_id.raw.has_design);
            // END NEW
            if (payload) {
                ...
                Object.assign(values, {
                    design_id: payload.design_id ?? false,
    ...
    async openConfigurator(product,has_design = false) {
        if(!has_design)
            return super.openConfigurator(product)
